...
const Dealership = ({ isAdminAccount, isDealerAccount, getSalesmanObj }) => {
        ...
        >
          ...
          <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
            ...
            {!loading && dealership && <>
              ...
              <div style={{ marginBottom: '2px' }}>Lifetime total transactions {dealership.lifetimeTotalTransactions}</div>
              <div style={{ marginBottom: '2px' }}>grossSalesToCustomers {dealership.grossSalesToCustomers}</div>
              <div style={{ marginBottom: '2px' }}>grossProfitForDealership {dealership.grossProfitForDealership}</div>
                      ...
                      <Box sx={{ width: '100%', overflow: 'hidden' }}>
                        <TableContainer>
                          <Table stickyHeader size="small">
                            ...
                            <TableBody>
                              {dealership.Salespeople.items.map((row, index) => (
                                <TableRow key={row.id}>
                                  <TableCell>{index + 1}</TableCell>
                                  <TableCell><Link href="#" onClick={() => navigate('/salesperson/' + row.id)} >{row.firstName} {row.lastName}</Link></TableCell>
                                  ...
                                  <TableCell>{row.grossProfitForDealership}</TableCell>
                      ...
                      <Box sx={{ width: '100%', overflow: 'hidden' }}>
                        <TableContainer>
                          <Table stickyHeader size="small">
                            ...
                            <TableBody>
                              {dealership.Invoices.items.map((row) => (
                                <TableRow key={row.id}>
                                  <TableCell><Link href="#" onClick={() => navigate('/invoice/' + row.id)} >{row.id}</Link></TableCell>
                                  <TableCell>{row.dueDate}</TableCell>
                                  <TableCell>{row.pastDue ? 'Past' : ''}</TableCell>
                                  <TableCell>{row.isPaid ? 'Paid' : ''}</TableCell>
